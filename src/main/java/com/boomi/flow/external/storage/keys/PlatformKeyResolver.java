...
public class PlatformKeyResolver implements DecryptionKeyResolver, VerificationKeyResolver {
    ...
    private Key resolveKey(String id) throws UnresolvableKeyException {
        var key = keyRepository.findPlatformKey(UUID.fromString(id));
        if (key == null) {
            throw new UnresolvableKeyException("Unable to resolve a platform key with the ID " + id);
        }
